...
- [ ] Add `/api/db/weeks/:weekId/summary` with aggregate counts and three ranked previews per day.
- [ ] Remove the unpaginated specified-week branch from `/api/db/cards` and add `dayIndex` filtering.
- [ ] Add an authenticated smoke test for summary limits, weekly paging and day filtering.

### Task 2: Diary Summary and Silent Refresh

**Files:** `miniprogram/app/pages/diary/index.js`

- [ ] Replace the 200-card request with the summary endpoint.
- [ ] Preserve current data on every `onShow` after the first successful load.
...
- [ ] Add 20-row reset/append loaders with ID de-duplication and `hasMore` guards.
- [ ] Bind page or scroll-view bottom events and show compact loading/end states.
- [ ] Make book `onShow` refresh silent when existing data is present.

### Task 4: Search Debounce and Paging

**Files:** `miniprogram/app/pages/search/index.js`, `index.wxml`, `index.wxss`

- [ ] Debounce text input by 300ms and increment a request sequence for each reset.
- [ ] Load and append 20-row pages, ignoring stale responses.

### Task 5: Favorite Paging and Return-State Preservation

**Files:** `miniprogram/app/pages/me/index.js`, `index.wxml`, `index.wxss`

- [ ] Load favorites in 12-row pages and append on bottom only when 收藏 is active.
- [ ] Keep books, drafts and favorites visible during later `onShow` refreshes.

### Task 6: Verification and Publish

**Files:** all files above

- [ ] Run `npm run mini:p2:smoke`, all mini-program JavaScript checks, `npm run lint`, `npm run build`, auth smoke and combo smoke.
- [ ] Confirm no AppID is staged, commit the implementation, and push `main`.
